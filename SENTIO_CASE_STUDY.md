...
Iteration was intentional: I let the stack and constraints change while keeping the core product lens constant.
...
- Renamed from **Product Pulse** to **Sentio** with the tagline:  
  **“Understand what your product team really sees.”**
...
- Made `/checkin` visually and emotionally inviting:
...
- Added **seeding scripts** to generate synthetic data for three archetypal teams:
  - Nebula Labs (declining)
  - Orbit Engine (volatile)
  - Patchwork Co (mediocre but consistent)
...
- **Why it was difficult**  
  Throwing away a working stack under a 2‑week window is painful; I had to resist the sunk-cost bias.
- **How it was solved**  
...
- **What I learned**  
  Leaning into the framework’s env conventions (`$env/*`) saves time and makes deployment (Netlify) less fragile.
...
- **What I learned**  
  Treat external integrations as hostile until proven friendly; validate URLs and consider response *shape*, not just `status`.

#### Challenge 4 — Reading from `#general` with a bot

- **What happened**  
  The Discord bot returned an array of messages with **empty content** — or worse, I saw empty lists.
- **Why it was difficult**  
...
- **How it was solved**
  ...
  - Relaxed the dashboard loader to support both string and `{content,…}` shapes.
- **What I learned**  
  When integrating with third-party APIs, add a thin, explicit diagnostic surface before you try to design clever features on top.
...
- **What I learned**  
  UX tone and visual treatment can dramatically change the perceived weight of a task without changing any underlying fields.
...
- I used AI to:
  - Sanity-check architecture decisions (e.g. where to put logic in SvelteKit).
  - Draft and refine server routes, schema definitions, and UI components.
  - Debug hairy integration issues (Neon, Better Auth, Discord).
- I remained responsible for:
  - The product framing (what’s in scope, what’s ritual vs. feature creep).
  - Trade-off calls (e.g. choosing Discord over Slack, read-only bot vs. full sync).
  - Final API shapes, data modeling, and UX details.

The loop looked like: **sketch intent → co-design implementation with AI → run, test, and adjust**. Logs and errors, not guesses, drove most of the hard fixes.
...
- Exposed more nuance (per-member, per-week, sentiment tags).
...
In a real team context, Sentio would reduce “surprise” risk: leaders would see execution risk, morale dips, and blockers before sprint reviews or incident reports catch up.
...
3. **Integrations need guardrails, not just happy paths.**  
   Validating Discord webhook URLs and detecting HTML responses prevented “silent successes” that weren’t successes at all.
...
6. **Delight is often a thin layer, but a meaningful one.**  
   The dark neon theme, glowing scores, emojis, and tag chips are thin layers of CSS and copy, but they change how the tool *feels* to use — especially for something as sensitive as team sentiment.
...
The current capstone version is intentionally a slice, not a platform — but it already demonstrates the throughline I care about: **connecting how a team feels with what they say and what they’re shipping.**
